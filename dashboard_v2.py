...
# @st.cache
def backtest(sport):
    # international_basketball_league = ['EuroLeague', 'EuroCup', 'NBLAustralia', 'FranceLNB', 'VTBUnited', 'SpainACB', 'GreekBasketball']
    try:
    ...
    with st.spinner('Wait for it...'):
        ...
        for index, row in df.iterrows():
            ...
            if row['Predicted_Spread'] > 0:
                final.loc[index, 'Spread/ML'] = f'{row["Away_Team"]} win by {round(abs(row["Predicted_Spread"]))}'
            else:
                final.loc[index, 'Spread/ML'] = f'{row["Home_Team"]} win by {round(abs(row["Predicted_Spread"]))}'
